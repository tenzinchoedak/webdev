...
function UserList(props: UserProps){
	return (
	 <article>
		 <h1>{props.Use.id}</h1>
		 <img src={props.Use.avatar} />
		 <h2>{props.Use.first_name} {props.Use.last_name}</h2>
		 <p>{props.Use.age}</p>
		 <p>{props.Use.city}</p>
		 <p>{props.Use.ip}</p>
		 <input type="checkbox" checked={props.Use.isAdmin} />
	 </article>
	);
}
function App(){
	const discourse: UserInfo = {
		id: 1,
  ...
  return <>
	<section>
		<p id="name">Adult</p>
		{users.filter(user => user.age >= 18).map(user => <UserList key={user.id} Use={user} />)}
		</section>,
	<section>
		<p id="name">Kids</p>
		{users.filter(user => user.age < 18).map(user => <UserList key={user.id} Use={user} />)}
